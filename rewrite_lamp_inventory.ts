...
export function LampInventory({ assets, itemTypes, buoys = [] }: LampInventoryProps) {
    ...
    const handleMinStockChange = async (itemType: any, change: number) => {
        if (!itemType || !itemType.id || String(itemType.id).startsWith('custom-')) return;
        const currentMin = itemType.min_stock_level || 0;
        const newMin = Math.max(0, currentMin + change);
        
        const id = \`\${itemType.id}-min-\${change > 0 ? 'inc' : 'dec'}\`;
        setLoadingMin(id);
        
        try {
            await updateItemMinStock(itemType.id, newMin);
            itemType.min_stock_level = newMin; // mutate local for instant feedback
        } finally {
            setLoadingMin(null);
        }
    };
    ...
    const groupedRows = useMemo(() => {
        const groups = new Map<string, {
            ...
            broken: number;
            lost: number;
            matchingAssets: any[]; // Only assets matching search & filters
            allAssets: any[];      // All assets in this group for true totals
        }>();

        assets.forEach(asset => {
            const key = asset.item_id || 'custom_' + asset.id;
            
            if (!groups.has(key)) {
                const type = itemTypes.find(t => t.id === key);
                const colorValue = type?.name || asset.metadata?.color || asset.metadata?.lamp_color || 'Onbekend';
        ...
        // Also add empty types that aren't represented but exist (so they can set minimum stock)
    ...
    // Stats for summary cards (calculated over all matching assets)
    const stats = useMemo(() => {
        const flatMatching = assets.filter(matchesFilters);
        return {
    ...
    const getColorDot = (color: string) => {
        ...
        if (lower.includes('blauw')) return 'bg-blue-500';
        return 'bg-gray-400';
    };
                    ...
                    <table className="w-full text-left border-collapse min-w-[800px]">
                        <thead>
                            <tr className="bg-app-bg/50 border-b border-app-border">
                                ...
                                <th className="px-6 py-4 text-xs font-bold text-app-text-secondary uppercase tracking-wider">Kleur / Type</th>
                        ...
                        <tbody className="divide-y divide-app-border">
                            {groupedRows.length > 0 ? (
                                groupedRows.map((group) => {
                                    ...
                                    const minStock = group.itemType?.min_stock_level || 0;
                                            ...
                                            >
                                                ...
                                                <td className="px-6 py-4 text-center">
                                                    <div className="flex items-center justify-center gap-1.5 opacity-40 group-hover:opacity-100 transition-opacity">
                                                        <button
                                                            onClick={(e) => { e.stopPropagation(); handleMinStockChange(group.itemType, -1); }}
                                                            disabled={!!loadingMin || !group.itemType?.id || String(group.itemType.id).startsWith('custom') || minStock <= 0}
                                                        ...
                                                        <span className="w-4 text-center font-mono font-medium text-xs text-app-text-secondary">
                                                            {loadingMin?.includes(\`\${group.itemType?.id}-min-\`) ? <Loader2 className="w-3 h-3 animate-spin mx-auto" /> : minStock}
                                                        </span>
                                                        <button
                                                            onClick={(e) => { e.stopPropagation(); handleMinStockChange(group.itemType, 1); }}
                                                            disabled={!!loadingMin || !group.itemType?.id || String(group.itemType.id).startsWith('custom')}
                                                            className="w-5 h-5 flex items-center justify-center rounded border border-app-border hover:bg-app-bg text-app-text-secondary disabled:opacity-30 transition-all font-mono text-xs"
                                                        >
                                                            +
                                                        </button>
                                                                                ...
                                                                                <tr key={asset.id} className="hover:bg-app-surface-hover/50 group/child transition-colors">
                                                                                    ...
                                                                                    <td className="py-3 text-center">
                                                                                        <div className="flex items-center justify-center gap-2">
                                                                                            {asset.metadata?.ble ? <Bluetooth className="w-3.5 h-3.5 text-blue-500" title="BLE Geactiveerd" /> : <BluetoothOff className="w-3 h-3 text-gray-400 opacity-20" />}
                                                                                            {asset.metadata?.gps ? <Navigation className="w-3.5 h-3.5 text-indigo-500" title="GPS Geactiveerd" /> : <Navigation className="w-3 h-3 text-gray-400 opacity-20" />}
                                                                                        </div>
                                                                                    </td>
...
fs.writeFileSync('src/components/LampInventory.tsx', updatedCode);
